...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>ducblixfruit chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "ducblixfruit", HidePremium = false, IntroText = "ducblixfruit⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("ducblixfruit SCRIPT")
W:AddLabel("Discordducblixfruit")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "ducblixfruit(Noitification)",
